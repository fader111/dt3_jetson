...
window.onload = function () {
    ...
    var settingsFromServer;     //структура, содержащая настройки детектора, в т.ч. данные калибровочного полигона
    var calibPoints;            // массив точек калибровочного полигона, вида [[x,y],[..],[..],[..]]
    var calib_path;             // path для ппостроения каалибровочного полигона вида ["M200,100 L600,100 L600,500 L200,500 Z"]
    var polygonNumber;
    ...
    var editMode = 0;           // режин настройки зон детектирования
    var calibrationMode = 0;    // режим калибровки расстояний на дороге
    ...
    function getSettingsFromServer() {
        //gets settings from server when page load
        $.ajax({
            type: "POST",
            url: "/getSettingsFromServer",
            //data: $('form').serialize(),
            data: "",
            success: function (response) {
                __setts = jQuery.parseJSON(response);
                calibPoints = JSON.parse(__setts).calibration;
                // console.log('response PARSED settings', __setts);
                // console.log('calibPoints ---', calibPoints);
                return 0;
            },
            error: function (error) {
                console.log(error);
    ...
    settingsFromServer = getSettingsFromServer(); // забирает настройки с сервера
    ...
    ff = function (k, i) {
        calibHolder.style.visibility = "hidden";
        polygones.push(
    ...
    /*function drawCalibPoly(){ // КАНДИДАТ НА УДАЛЕНИЕ, его функционал лежит в ф-ции calibrationButton.onclick
        // рисует калибровочный полигон когда заходим в калибровку
        //calib_path = ["M200,100 L600,100 L600,500 L200,500 Z"];//[];
        if (calibPoints !== undefined){
            calib_path = ["M", calibPoints[0][0], calibPoints[0][1], "L", calibPoints[1][0], 
                            calibPoints[1][1], "L", calibPoints[2][0], calibPoints[2][1], "L", 
                            calibPoints[3][0], calibPoints[3][1], "Z"];
            console.log('calib_path',calib_path);
            ffCalib(); 
        }              
    }*/
    ...
    // setTimeout(drawCalibPoly, serverAnswerDelTime+5);
    ...
    function winOnmouseUp() { // каждый раз при поднятии мышки данные полигонов сохраняются на сервере.
        if (editMode == 1) {
            //                    //sendPolyToServer(req); // нафик не надо это делать, они и так отправляются каждый раз, 
            // когда что-то делаешь с полигонами
        }
        ;
    ...
    // draw calibration polygon over the road
    var calibrationButton = document.getElementById('calibrationButton');
    var saveCalibButton = document.getElementById('saveCalibButton');
    ...
    // getSettingsFromServer(); //для обновления точек calibPoints, a то старые будут, если второй раз за сессию нажимаешь. по ходу не робит хз почему
    ffCalib = function (k = 1, i = 0) { // k, i  тут не нужны, это осталось при копировании от старого функционала. надо вычистить. 

        if (calibPoints !== undefined) {
            arr_calibPoints = JSON.parse(calibPoints);
            // console.log('arr_calibPoints!!!',arr_calibPoints);
            calib_path = ["M", arr_calibPoints[0][0], arr_calibPoints[0][1], "L", arr_calibPoints[1][0],
                arr_calibPoints[1][1], "L", arr_calibPoints[2][0], arr_calibPoints[2][1], "L",
                arr_calibPoints[3][0], arr_calibPoints[3][1], "Z"];
        };
        // console.log('calib_path--', calib_path, "calibPolygone", calibPolygone);
        // следующее здесь осталось от версии, когда калибр полигон строился и удалялся, теперь он строится раз в сессию
        // if (calibPolygone.remove !== undefined) {
        //     console.log('clear calibr!!!! pcal polygone = ', calibPolygone);
        //     calibPolygone.remove();
        // }
        // console.log('калибре запустиле, calib_path=', calib_path);

        calibPolygone =
            rc.path(calib_path).attr({ stroke: "blue", fill: "blue", opacity: 0.5 })
                .data("polygonNumber", i)
                .mousedown(function () {
                ...
                .mouseup(function () {
                    if (calibrationMode == 1) {
                        ...
                        this.attr({ path: path_ });
                        // для всех 4-х углов полигона рисуем квадратики за которые будем таскать
                        for (ii = calibRects.length - 1; ii >= 0; ii--) calibRects.pop();//splice(i,1); //удалем старые квадратики перед созд. новых
                        for (ii = 0; ii < 4; ii++) {
                            var internalFlag = 0;
                            // создать квадратики по углам полигона
                            var rec = calibRects.push(rc.rect(path_[ii][1] - 10, path_[ii][2] - 10, 20, 20).attr({ stroke: "blue", fill: "yellow", opacity: 0.5 }))
                                .drag(function (dx, dy, x, y) { // таскать за квадратики по углам
                                    leftCornerShiftX = Math.round(document.getElementById('img_div').getBoundingClientRect().left);
                                    leftCornerShiftY = Math.round(document.getElementById('img_div').getBoundingClientRect().top);
                                    //internalFlag=1;
                                    ...
                                    //this.attr({height:20,width:20,x:x-10,y:y-10});
                                    this.attr({ height: 20, width: 20 });
                                    var new_path = calibPolygone.attr("path");
                                    var numOfRect;
                                    for (j = 0; j < calibRects.length; j++) {
                                        if (calibRects[j].attr("x") == this.attr("x") & calibRects[j].attr("y") == this.attr("y")) numOfRect = j;
                                        //console.log("numOfRect=",numOfRect);
                                    }
                                    if (x > 0 && x < W) {
                        ...
                        // save changes after each mouse up
                        pushSettings();
                    }
                    else {
                        // notEditModeAlert();
    ...
    setTimeout(ffCalib, serverAnswerDelTime + 25); //прорисовали полигон калибровки, без таймаута не успевает

    calibrationButton.onclick = function () {
        // console.log('нажали кнопке калибер батон ', calibPoints)
        calibrationMode = 1;
        saveCalibButton.style.visibility = "visible";
        calibHolder.style.visibility = "visible";
    };
    ...
    function pushSettings() {
        ...
        if (calibPolygone[0] != null) { //так можо проверить если полигон удалили даблкликом
            ...
            let x1 = calibPolygone.attr("path")[0][1]; //координаты точек полигона
            let y1 = calibPolygone.attr("path")[0][2];
            let x2 = calibPolygone.attr("path")[1][1];
            let y2 = calibPolygone.attr("path")[1][2];
            let x3 = calibPolygone.attr("path")[2][1];
            let y3 = calibPolygone.attr("path")[2][2];
            let x4 = calibPolygone.attr("path")[3][1];
            let y4 = calibPolygone.attr("path")[3][2];

            calibrationPoints = [[x1, y1], [x2, y2], [x3, y3], [x4, y4]];
        }
        else {
            calibrationPoints = [[200, 100], [600, 100], [600, 500], [200, 500]]; // дефолтный полигон посередине
        }
    ...
    saveSettingsButton.onclick = pushSettings;

    saveCalibButton.onclick = function () {
        // console.log('saveCalibButtton onclick happpend');
        calibrationMode = 0;
        if (calibRects != null) calibRects.remove();
        pushSettings();
        // if (calibPolygone != null) calibPolygone.remove();
        calibHolder.style.visibility = "hidden";

        /* ...... */

    };
    // убрать кнопку если калибровка закончена после того как с нее убрали курсор
    saveCalibButton.onmouseout = function () {
        if (calibrationMode == 0)
            saveCalibButton.style.visibility = "hidden";
    }
}
